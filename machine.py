...
class DataPath:
    ...
    def latch_program_counter(self, sel_next):
        ...
        assert self.registers["rx1"] < len(
            self.data_mem
        ), "Out of instruction memory: {}".format(self.registers["rx1"])
    ...
    def latch_data_mem_counter(self, sel_next):
        ...
        assert self.registers["rx2"] < len(
            self.instr_mem
        ), "Out of data memory: {}".format(self.registers["rx2"])
...
class ControlUnit:
    ...
    def decode_and_execute_instruction(self):
        ...
        if opcode == Opcode.MOV.value:
            if cur_instr["arg2"] == "rx2":
                self.data_path.val_to_mov = self.data_path.data_mem[
                    self.data_path.registers.get("rx2")
                ]
            else:
                if cur_instr["arg2"] in self.data_path.registers:
                    self.data_path.val_to_mov = self.data_path.registers.get(
                        cur_instr["arg2"]
                    )
                else:
        ...
        if opcode in {
            ...
            if opcode == Opcode.ADD.value:
                self.data_path.val_to_mov = self.alu.add(
                    self.data_path.registers.get(res_reg),
                    self.data_path.registers.get(cur_instr["arg2"]),
                )
            if opcode == Opcode.SUB.value:
                self.data_path.val_to_mov = self.alu.sub(
                    self.data_path.registers.get(res_reg),
                    self.data_path.registers.get(cur_instr["arg2"]),
                )
            if opcode == Opcode.MUL.value:
                self.data_path.val_to_mov = self.alu.mul(
                    self.data_path.registers.get(res_reg),
                    self.data_path.registers.get(cur_instr["arg2"]),
                )
            if opcode == Opcode.DIV.value:
                self.data_path.val_to_mov = self.alu.div(
                    self.data_path.registers.get(res_reg),
                    self.data_path.registers.get(cur_instr["arg2"]),
                )
            if opcode == Opcode.MOD.value:
                self.data_path.val_to_mov = self.alu.mod(
                    self.data_path.registers.get(res_reg),
                    self.data_path.registers.get(cur_instr["arg2"]),
                )
            self.tick()
    ...
    def __repr__(self):
        return (
            "{{TICK: {}, RX1: {}, RX2: {}, RX3: {}, RX4: {}, RX5: {}, RX6: {}, RX7: {}, RX8: {}, RX9: {}, "
...
def simulation(code, input_token, instr_limit, iter_limit):
    ...
    try:
        ...
        logging.error(
            "Too many instructions. Please, increase size of instruction memory."
        )

    try:
        while True:
            assert iter_limit > instr_counter, (
                "Too many iterations. "
...
def main(args):
    assert (
        len(args) == 2
    ), "Wrong amount of arguments. Please, read instruction carefully."
    ...
    output, ticks, instr_amount = simulation(
        code, input_token, instr_limit=2048, iter_limit=100000000
    )
    logging.info("output:  %s, ticks: %s", repr(output), repr(ticks))
    print(
        "Output buffer: {} | ticks: {} | amount_instr: {}".format(
            "".join(map(str, output)), repr(ticks), repr(instr_amount)
        )
    )
